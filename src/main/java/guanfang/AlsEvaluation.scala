...
object AlsEvaluation {
  ...
  def PrepareData(): (RDD[Rating], RDD[Rating], RDD[Rating]) = {
    ...
    val sc: SparkContext = new SparkContext(new SparkConf().setAppName("RDF").setMaster("local[*]"))
    //----------------------1.创建用户评分数据-------------
    print("开始读取用户评分数据...")
    val DataDir = "/home/rjxy/IdeaProjects/spark/spark_mllib_course/src/main/resources/ml-100k"
